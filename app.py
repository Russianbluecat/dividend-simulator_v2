...
"""
        ...
        # 자세한 오류 정보 (디버깅용)
        with st.expander("🔍 오류 세부사항 (기술지원용)"):
            st.code(f"""
오류 타입: {type(e).__name__}
오류 메시지: {error_message}
배당주: {dividend_stock if 'dividend_stock' in locals() else 'N/A'}
투자 대상: {invest_stock if 'invest_stock' in locals() else 'N/A'}
시작일: {start_date if 'start_date' in locals() else 'N/A'}
            """)
        
        st.info("""
        **문제 해결 방법:**
        1. 티커 심볼이 정확한지 확인하세요
        2. 시작 날짜가 너무 과거이거나 미래가 아닌지 확인하세요
        3. 인터넷 연결이 안정적인지 확인하세요
        4. 잠시 후 다시 시도해보세요
        5. 캐시 정리 버튼을 눌러보세요
        """)
